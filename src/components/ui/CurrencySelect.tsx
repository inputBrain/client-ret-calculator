...
export default function CurrencySelect({
    ...
    const options = (Object.keys(CURRENCY_META) as Currency[]).filter(
        (code) => !exclude.includes(code)
    );

    return (
        <Select.Root value={value} onValueChange={(v) => onValueChangeAction(v as Currency)}>
            <Select.Trigger className="inline-flex h-8 items-center gap-2 rounded-full px-3 text-sm text-indigo-700 hover:text-indigo-800 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-200" aria-label="Currency">
